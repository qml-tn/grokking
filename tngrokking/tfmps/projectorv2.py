...
from tngrokking.tfmps.embeddings import FourierEmbedding


class ProjectorV2(tf.keras.layers.Layer):
    def __init__(self, D, d=2, S=2, nc=2, stddev=0.5):
    ...
    def build(self, input_shape):
        ...
        self.MPS_output = tf.Variable(tf.random.normal(shape=(
            self.Nout, self.nc, D, D, self.d, self.d), stddev=stddev), name="mps_output", trainable=True)
    ...
    def log_norm(self):
        ...
        for i in range(n):
            ...
            if (i % S == 0 or i == n-1):
                ...
                Al = tf.einsum("cdu,cdkij->ckuij", Al, M)
                Al = tf.einsum("ckuij,culij->ckl", Al, M)
            else:
                M = self.MPS_input[i_in]
                Al = tf.einsum("cdu,cdki->ckui", Al, M)
                Al = tf.einsum("ckui,culi->ckl", Al, M)
                i_in += 1
        scals.append(Al[:, 0, 0])
        ...
        for i in range(k+1):
            ...
            if (i % S == 0 or i == n-1):
                if i == n-1:
                    ...
                    M = self.MPS_output[int(i//S)]
                if sampQ[i]:
                    M = tf.einsum("clrij,i->clrj", M, samp[i])
