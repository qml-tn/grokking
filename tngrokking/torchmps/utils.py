...
from tngrokking.torchmps.embeddings import image_embedding
...
def load_HV_data(length):
    ...
    if length > 14:
        logging.warning("load_HV_data will generate {} images, "
...
def get_test_loader(datadir, dataset, crop, bs):
...
def evaluate(datadir, dataset, modelsdir, model_name, bs, device="cpu"):
    model = torch.load(os.path.join(modelsdir, model_name),
    ...
    test_loader, num_batches = get_test_loader(datadir, dataset, crop, bs)
...
#         logging.info("start evaluation")
        for inputs, labels in tqdm(test_loader):
            ...
            inputs = image_embedding(inputs, aug_phi=0, embedding=config['embedding'])

            if config['permute']:
                inputs = inputs[:, config["inds"], :]
