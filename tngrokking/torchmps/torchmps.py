...
from tngrokking.torchmps.utils import init_tensor, svd_flex
from tngrokking.torchmps.contractables import SingleMat, MatRegion, OutputCore, ContractableList, \
    EdgeVec, OutputMat
import logging
import os


def left_canonical_R(mps, periodic=True):
    D = mps.shape[1]
    d = mps.shape[3]
    device = "cpu"
    ...
    n = len(mps)
    for i in range(n):
        A = torch.einsum("ij,jkl->ilk", R, mps[i])
        A = torch.reshape(A, [-1, D])
        A = A/(torch.linalg.norm(A) + 1e-10)
        # We are adding a small random vector in order to avoid NaNs in the backward pass
        _, R = torch.qr(A + 1e-10*torch.rand(A.shape, device=device))
    return R


def right_canonical_R(mps, periodic=True):
    D = mps.shape[1]
    d = mps.shape[3]
    device = "cpu"
    if torch.cuda.is_available():
        device = "cuda"
    Rt = torch.eye(D, device=device)
    if not periodic:
        Rt = Rt*0
        Rt[0, 0] = 1.0
    n = len(mps)
    for i in range(n-1, -1, -1):
        A = torch.einsum("ijk,lj->kli", mps[i], Rt)
        A = torch.reshape(A, [-1, D])
        A = A/(torch.linalg.norm(A)+1e-10)
        # We are adding a small random vector in order to avoid NaNs in the backward pass
        _, Rt = torch.qr(A + 1e-10*torch.rand(A.shape, device=device))
    return torch.transpose(Rt, 1, 0)


def left_canonical_state(mps, periodic=True):
    D = mps.shape[1]
    d = mps.shape[3]
    device = "cpu"
    if torch.cuda.is_available():
    ...
    for i in range(n):
        ...
        Q, R = torch.qr(A)
        mpsl.append(torch.reshape(Q, [D, d, -1]))
    mpsl.append(R)
    return mpsl


def right_canonical_state(mps, periodic=True):
    D = mps.shape[1]
    d = mps.shape[3]
    device = "cpu"
    if torch.cuda.is_available():
    ...
    for i in range(n-1, -1, -1):
        ...
        A = A/torch.linalg.norm(A)
        Qt, Rt = torch.qr(A)
        mpsr = [torch.reshape(torch.transpose(Qt, 1, 0), [D, d, -1])] + mpsr

    mpsr = [torch.transpose(Rt, 1, 0)] + mpsr
    return mpsr


def entropies_MPS(model):
    periodic = model.config["periodic"]
    mps = model.linear_region
    left = (mps.module_list[0].tensor +
            mps.module_list[0].bias_mat.unsqueeze(-1))
    right = (mps.module_list[2].tensor +
             mps.module_list[2].bias_mat.unsqueeze(-1))
    outsite = mps.module_list[1].tensor

    D = model.bond_dim
    d = model.feature_dim
    nout = model.output_dim

    mpsl = left_canonical_state(left, periodic)
    mpsr = right_canonical_state(right, periodic)

    # Entropy at the middle
    Rm = torch.einsum("kl,olr,rs->oks", mpsl[-1], outsite, mpsr[0])
    Rm = Rm/torch.linalg.norm(Rm, dim=(1, 2), keepdim=True)
    _, s, _ = torch.svd(Rm)
    ent = -torch.sum(s**2*torch.log(s**2), dim=1)
    ents = [ent]

    # Right part
    Rl = Rm
    for Qr in mpsr[1:]:
        A = torch.einsum("okl,lir->okir", Rl, Qr)
        A = torch.reshape(A, [nout, -1, D])
        _, s, vh = torch.svd(A)
        ent = -torch.sum(s**2*torch.log(s**2), axis=1)
        ents.append(ent)
        Rl = torch.einsum("ol,olr->olr", s, vh)

    # Left part
    Rr = Rm
    for Ql in reversed(mpsl[:-1]):
        A = torch.einsum("lir,ors->olis",  Ql, Rr)
        A = torch.reshape(A, [nout, D, -1])
        u, s, _ = torch.svd(A)
        ent = -torch.sum(s**2*torch.log(s**2), dim=1)
        ents = [ent] + ents
        Rr = torch.einsum("olr,or->olr", u, s)
    return ents


def entropy(model, seq_len=60):
    if type(model) == MPS:
        return entropy_MPS(model)
    if type(model) == TI_MPS:
        return entropy_TI_MPS(model, seq_len=seq_len)
    return 0


def entropies(model, seq_len=60):
    if type(model) == MPS:
        return entropies_MPS(model)
    if type(model) == TI_MPS:
        return entropies_TI_MPS(model, seq_len=seq_len)
    return 0


def entropy_TI_MPS(model, seq_len=60):
    ents = entropies_TI_MPS(model, seq_len=seq_len)
    return torch.mean(ents[len(ents)//2])


def entropies_TI_MPS(model, seq_len=25*25):
    # Prepend an EdgeVec and append an OutputMat
    mps = model.core_tensor
    init_vec = model.init_vector.vec
    term_mat = model.terminal_mat.mat
    ...
    # Left canonical form
    A = torch.einsum("l,lri->ir", init_vec, mps)
    A = A/torch.linalg.norm(A)
    mpsl = []
    for i in range(1, seq_len+1):
        dims = A.shape
        Ql, R = torch.qr(A)
        A = torch.einsum("il,lrj->ijr", R, mps)
        A = torch.reshape(A, [-1, D])
        A = A/torch.linalg.norm(A)
        Dr = R.shape[0]
        mpsl.append(torch.reshape(Ql, [-1, d, Dr]))

    ents = []
    A = torch.reshape(A, [Dr, d, D])
    Ar = torch.einsum("lir,ro->oli", A, term_mat)
    for Ql in reversed(mpsl):
        Ar = Ar/torch.linalg.norm(Ar, dim=(1, 2), keepdim=True)
        u, s, _ = torch.svd(Ar)
        ent = -torch.sum(s**2*torch.log(s**2), dim=1)
        ents.append(ent)
        Dl = Ql.shape[0]
        u = torch.einsum("ora,oa->ora", u, s)
        Ar = torch.einsum("lir,ora->olia", Ql, u)
        Ar = torch.reshape(Ar, [nout, Dl, -1])
    return ents


def entropy_MPS(model):
    periodic = model.config["periodic"]
    mps = model.linear_region
    left = (mps.module_list[0].tensor +
            mps.module_list[0].bias_mat.unsqueeze(-1))
    ...
    Rl = left_canonical_R(left, periodic)
    Rr = right_canonical_R(right, periodic)
    Aout = torch.einsum("il,olr,rj->oij", Rl, outsite, Rr)
    ent = []
    for i in range(len(Aout)):
        A = Aout[i]/torch.linalg.norm(Aout[i])
        s = torch.svd(A)[1]
        ent.append(-torch.sum(s**2*torch.log(s**2)))
    return torch.mean(torch.stack(ent))


def build_mps_model(*args, **config):
    logging.info(f"Initializing the MPS.")
    model_path = config["model_path"]
    learn_rate = config["lr"]
    reset_early_stopping = config["reset_early_stopping"]
    checkpoint = config["checkpoint"]

    config["best_val_acc"] = 0.0
    config["best_val_acc_epoch"] = 0
    config["epoch"] = 0
    if os.path.exists(model_path):
        logging.info(
            "MPS model already exists. Loading the old checkpoint and continuing training.")
        mps = torch.load(model_path)
        if hasattr(mps, "config"):
            lr = mps.config["lr"]
            logging.info(
                f"Resetting the learning rate from {learn_rate} to {lr}.")
            config["lr"] = lr
        if hasattr(mps, "best_val_acc_epoch") and hasattr(mps, "epoch"):
            config["epoch"] = mps.epoch
            if reset_early_stopping:
                config['best_val_acc_epoch'] = mps.epoch
            else:
                config['best_val_acc_epoch'] = mps.best_val_acc_epoch
            logging.info(f"Epoch: {config['epoch']}")
            logging.info(f"Best epoch: {config['best_val_acc_epoch']}")
        if hasattr(mps, "best_val_acc"):
            config['best_val_acc'] = mps.best_val_acc
            logging.info(f'Best acc: {config["best_val_acc"]}')
        else:
            logging.info(
        ...
        logging.info(
            f"Starting from a checkpoint: {checkpoint}")
        mps = torch.load(checkpoint)
        # We setup only the initial accuracy if it exists in order to speed up hyperparameter search
        if hasattr(mps, "best_val_acc"):
            best_val_acc = mps.best_val_acc
            logging.info(f'Best acc: {best_val_acc}')
        # We override previous training parameters with the new ones since we are interested only in initial weights.
        # consistency check
        assert config["D"] == mps.config[
            "D"], f"Bond dimensions do not agree! specified {config['D']} checkpoint: {mps.config['D']}"
        assert config["crop"] == mps.config[
            "crop"], f"Corp sizes do not agree! specified {config['crop']}"
        mps.config = config
    else:
        # If no model is saved we create a new one.
        if config['ti']:
            mps = TI_MPS(output_dim=config['nclass'], bond_dim=config['D'], feature_dim=config['feature_dim'],
                         parallel_eval=False, fixed_ends=False, init_std=1e-9, use_bias=True, fixed_bias=True,
                         permute=config["inds"], config=config)
        else:
            mps = MPS(input_dim=config['input_dim'], output_dim=config['nclass'], bond_dim=config['D'],
                      adaptive_mode=False, periodic_bc=config['periodic'], feature_dim=config['feature_dim'],
                      permute=config['inds'], config=config)
...
class TI_MPS(nn.Module):
    ...
    def __init__(self, output_dim, bond_dim, feature_dim=2,
                 ...
                 use_bias=True, fixed_bias=True, permute=None, config=None):
        super().__init__()
...
class MPS(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, bond_dim, feature_dim=2,
                 ...
                 fixed_bias=True, cutoff=1e-10, merge_threshold=2000, permute=None, config=None):
        super().__init__()
